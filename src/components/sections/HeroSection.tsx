      ...
      <div ref={heroRef} className="relative min-h-[calc(100svh-4rem)] overflow-hidden">
        ...
        <div className="relative z-10 flex min-h-[calc(100svh-4rem)] items-start px-4 pb-16 pt-4 mobile:pb-20 mobile:pt-6 tablet:items-center tablet:px-8 tablet:pb-0 tablet:pt-0">
          <div className="mx-auto w-full max-w-5xl">
            <div className="grid grid-cols-1 gap-6 mobile:gap-8 desktop:grid-cols-[1.2fr_1fr] desktop:items-center desktop:gap-16">
              {/* Left: Name & Bio — no solid card, just content floating */}
              <div className="hero-text-content">
                <div>
                  <p className="hero-greeting mb-3 font-nav text-[10px] uppercase tracking-[0.3em] text-slate-light mobile:mb-4 mobile:text-xs">
                    Hello, I&apos;m
                  </p>
                  <h1 className="hero-name mb-3 font-heading text-[2.75rem] font-bold leading-[0.95] tracking-tight text-cream mobile:mb-4 mobile:text-5xl tablet:text-7xl desktop:text-8xl">
                    Akhila
                    <br />
                  ...
                  <div className="hero-cta flex flex-col gap-2 mobile:flex-row mobile:flex-wrap mobile:gap-3">
                    <button
                      ...
                      className="group inline-flex w-full items-center justify-center gap-2 rounded-full bg-orange px-7 py-2.5 text-center font-nav text-sm font-semibold uppercase tracking-wider text-base transition-all duration-300 hover:bg-orange-muted hover:shadow-[0_0_35px_rgba(255,133,51,0.35)] mobile:w-auto cursor-none">
                      Get in Touch
                    ...
                    <a href="https://linkedin.com/in/akhila-susarla-1803b41b6/" target="_blank" rel="noopener noreferrer"
                      className="group relative flex h-11 w-full items-center justify-center overflow-hidden rounded-full border border-slate/30 bg-base/30 px-7 font-nav text-sm uppercase tracking-wider text-slate-light backdrop-blur-md transition-all duration-300 hover:border-orange/40 mobile:w-auto cursor-none">
              ...
              <div className="hero-text-content">
                <div className="grid grid-cols-1 gap-3 mobile:grid-cols-2 mobile:gap-4 tablet:gap-6">
                  <div className="hero-stat rounded-2xl border border-orange/10 bg-base/30 p-4 backdrop-blur-md transition-all duration-300 hover:border-orange/25 hover:bg-base/40">
                    <p className="font-heading text-2xl font-bold text-orange mobile:text-4xl tablet:text-5xl">3+</p>
                    <p className="mt-2 font-nav text-xs uppercase tracking-wider text-cream/50">Years Experience</p>
                  </div>
                  <div className="hero-stat rounded-2xl border border-slate/15 bg-base/30 p-4 backdrop-blur-md transition-all duration-300 hover:border-slate/30 hover:bg-base/40">
                    <p className="font-heading text-2xl font-bold text-slate-light mobile:text-4xl tablet:text-5xl">&lt;2s
                      
                    </p>
                    <p className="mt-2 font-nav text-xs uppercase tracking-wider text-slate/80">Response Latency</p>
                  </div>
                  <div className="hero-stat rounded-2xl border border-slate/15 bg-base/30 p-4 backdrop-blur-md transition-all duration-300 hover:border-slate/30 hover:bg-base/40">
                    <p className="font-heading text-2xl font-bold text-slate-light mobile:text-4xl tablet:text-5xl">LoRA</p>
                    <p className="mt-2 font-nav text-xs uppercase tracking-wider text-slate/80">LLM Fine-tuning</p>
                  </div>
                  <div className="hero-stat rounded-2xl border border-orange/10 bg-base/30 p-4 backdrop-blur-md transition-all duration-300 hover:border-orange/25 hover:bg-base/40">
                    <p className="font-heading text-2xl font-bold text-orange mobile:text-4xl tablet:text-5xl">3.78</p>
